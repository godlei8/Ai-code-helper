...
基于 `Spring Boot + LangChain4j + DashScope + Vue 3` 的编程学习与求职辅导项目。  
...
│  ├─ ai/model/QwenChatModelConfig.java
...
- 普通对话：基于通义千问聊天模型回答编程和求职问题
- 流式对话：后端返回 SSE，前端按 chunk 实时渲染
- 会话记忆：通过 `memoryId` 区分不同聊天会话
- RAG 检索增强：读取 `src/main/resources/docs` 下的本地资料构建知识库
- 工具调用：通过 `InterviewQuestionTool` 从面试鸭抓取相关面试题
- MCP 集成：通过 `McpConfig` 对接智谱 MCP 工具
...
- DashScope key 用于普通对话、流式对话和 embedding
- BigModel key 用于 MCP
- `McpConfig` 当前是强依赖，如果 BigModel token 不可用，相关 Bean 可能影响服务启动
...
- [EmbeddingStoreConfig.java](src/main/java/com/example/work_helper/ai/EmbeddingStoreConfig.java)
  提供内存向量库实现
...
- MCP 服务是否能连通，直接受 BigModel token 和远端服务状态影响
